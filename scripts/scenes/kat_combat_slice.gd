...
func _ready() -> void:
	...
	_hud = KatCombatHud.new()
	_hud.name = "KatCombatHud"
	_hud.configure(_player)
	add_child(_hud)

	_player.combat_impact.connect(_impact_director.kat_combat_impact)
	_player.effect_requested.connect(_impact_director.play_effect)
	_player.audio_requested.connect(_impact_director.play_audio)
	_player.guard_impact.connect(_impact_director.play_guard_impact)
	_player.shield_visual_changed.connect(_on_shield_visual_changed)
	_player.announcement_requested.connect(_hud.announce)
	_player.defeated.connect(_on_player_defeated)
	_spawn_wave()
	_hud.announce("THE SHATTERED RELIQUARY")
	print("Arcane Impact Kat content slice ready.")
