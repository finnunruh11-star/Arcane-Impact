...
func _ready() -> void:
	...
	_hud = FinCombatHud.new()
	_hud.name = "FinCombatHud"
	...
	_spawn_wave()
	_hud.announce("TOOLS DRAWN FROM SHADOW")
	print("Arcane Impact Fin content slice ready.")
...
func _check_wave_clear() -> void:
	...
	if alive_count == 0 and _next_wave_timer <= 0.0:
		...
		_hud.announce("RELIQUARY OUTMANEUVERED")


func _on_player_defeated() -> void:
	_hud.announce("FIN'S VEIL IS TORN")
	_restart_timer = 2.6
