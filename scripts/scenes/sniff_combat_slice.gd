...
func _ready() -> void:
	...
	_hud = SniffCombatHud.new()
	_hud.name = "SniffCombatHud"
	_hud.configure(_player)
	add_child(_hud)

	_player.combat_impact.connect(_impact_director.sniff_combat_impact)
	_player.effect_requested.connect(_impact_director.play_effect)
	_player.audio_requested.connect(_impact_director.play_audio)
	_player.lightning_arc_requested.connect(_impact_director.play_lightning_arc)
	_player.thunder_burst_requested.connect(_impact_director.play_thunder_burst)
	_player.announcement_requested.connect(_hud.announce)
	_player.defeated.connect(_on_player_defeated)
	_spawn_wave()
	_hud.announce("TEMPEST AT THE RELIQUARY")
	print("Arcane Impact Sniff content slice ready.")
...
func _check_wave_clear() -> void:
	...
	if alive_count == 0 and _next_wave_timer <= 0.0:
		...
		_hud.announce("RELIQUARY CLEARED")


func _on_player_defeated() -> void:
	_hud.announce("SNIFF BURNS OUT")
	_restart_timer = 2.6